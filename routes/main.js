...
router.post('/', async function(req,res){
  try {
    let { Age, Gender, Family_Diabetes, highBP, PhysicallyActive,
      Smoking, Alcohol, RegularMedicine, JunkFood, Stress, BPLevel,
      Prediabetes, UriationFreq, Pregnancies, BMI, Sleep, SleepSound
    } = req.body;
    

    if(!(typeof Family_Diabetes === "boolean") || 
    ...
    if(!(Number.isInteger(SleepSound)) || 
    ...
    if(!Age || !Gender || !PhysicallyActive || 
       !JunkFood || !Stress || !BPLevel ||
       !UriationFreq)
...
router.get("/get-data", async (req, res) => {
  ...
  try {
    ...
    if (!id)
    return res.status(400).json({ msg: "Must enter id" });
